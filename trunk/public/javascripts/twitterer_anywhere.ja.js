 ...
 */
var twitterer_anywhere_style; // falseを設定するとCSSでスタイルの指定が可能になる
var twitterer_anywhere_limit; // 表示件数
var twitterer_anywhere_collapse; // trueにすると、コメントの書いてないつぶやきを表示しない。指定しない場合は、表示件数を超えた場合のみ表示しない（現在利用不可）
var twitterer_anywhere_url; // 表示するURL 未指定の場合、このページ
...
function __twitterer_anywhere_receiver(json) {
	...
	if(json==null) {
	    html += "このエントリーのつぶやき (0) <a class=\"twitterer_anywhere_go\" href=\"http://pcod.no-ip.org/yats/search?query="+escapeHTML(twitterer_anywhere_url)+"\">twitter検索のページへ飛ぶ</a><br/>";
	    ...
	    html += "このエントリーのつぶやき ("+json.count+") <a class=\"twitterer_anywhere_go\" href=\"http://pcod.no-ip.org/yats/search?query="+escapeHTML(twitterer_anywhere_url)+"\">twitter検索のページへ飛ぶ</a><br/>";
		...
		if(twitterer_anywhere_collapse!=true || bookmark.comment!='') {
		    html += "<li><span class=\"__twitterer_anywhere_timestamp\">"+escapeHTML(t[0])+"年"+escapeHTML(t[1])+"月"+escapeHTML(t[2])+"日</span><img src=\"http://img.twitty.jp/twitter/user/"+escapeHTML(bookmark.user)+"/s.gif\" width=\"16\" height=\"16\"><a href=\"http://twitter.com/"+escapeHTML(bookmark.user)+"\" class=\"twitterer_anywhere_user\">"+escapeHTML(bookmark.user)+"</a>"+escapeHTML(bookmark.comment)+"<a href=\""+bookmark.url+"\">view</a></li>";
